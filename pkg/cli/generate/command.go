...
func Run(flags *Flags, config *raiden.Config, projectPath string, initialize bool) error {
	...
	go func() {
		...
		// generate types register
		GenerateLogger.Debug("start generate types register file")
		if err := generator.GenerateTypeRegister(projectPath, config.ProjectName, generator.Generate); err != nil {
			errChan <- err
		}
		GenerateLogger.Debug("finish generate types register file")

		GenerateLogger.Debug("start generate libs register file")
		if err := generator.GenerateLibRegister(projectPath, config.ProjectName, generator.Generate); err != nil {
			errChan <- err
		}
		GenerateLogger.Debug("finish generate libs register file")
