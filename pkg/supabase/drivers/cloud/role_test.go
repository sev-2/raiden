...
import (
	...
	cloud "github.com/sev-2/raiden/pkg/supabase/drivers/cloud"
...
func newTestConfig() *raiden.Config {
	return &raiden.Config{
		...
		SupabaseApiUrl:   "http://supabase.test",
...
func queryURL(cfg *raiden.Config) string {
	return fmt.Sprintf("%s/v1/projects/%s/database/query", cfg.SupabaseApiUrl, cfg.ProjectId)
}
...
func TestGetRolesSuccess(t *testing.T) {
	...
	url := queryURL(cfg)
	httpmock.RegisterResponder(http.MethodPost, url, func(req *http.Request) (*http.Response, error) {
		body, err := io.ReadAll(req.Body)
		require.NoError(t, err)
		assert.Contains(t, string(body), "rolname AS name")
		assert.Equal(t, fmt.Sprintf("Bearer %s", cfg.AccessToken), req.Header.Get("Authorization"))
		return httpmock.NewStringResponse(200, `[
            {"name":"role1","config":["search_path=public","statement_timeout=10"]},
            {"name":"role2"}
        ]`), nil
	})

	roles, err := cloud.GetRoles(cfg)
	require.NoError(t, err)
	require.Len(t, roles, 2)
	assert.Equal(t, "role1", roles[0].Name)
	assert.Equal(t, "public", roles[0].Config["search_path"])
	assert.Equal(t, "10", roles[0].Config["statement_timeout"])
	assert.Equal(t, "role2", roles[1].Name)
	assert.Nil(t, roles[1].Config)
}
...
func TestGetRolesServerError(t *testing.T) {
	...
	url := queryURL(cfg)
	httpmock.RegisterResponder(http.MethodPost, url, httpmock.NewStringResponder(500, `{}`))
...
func TestGetRolesInterceptorError(t *testing.T) {
	cfg := newTestConfig()
	setupHTTPMock(t)

	url := queryURL(cfg)
	httpmock.RegisterResponder(http.MethodPost, url, httpmock.NewStringResponder(200, `not-json`))
...
func TestGetRoleMembershipsSuccess(t *testing.T) {
	...
	httpmock.RegisterResponder(http.MethodPost, url, func(req *http.Request) (*http.Response, error) {
		...
		return httpmock.NewStringResponse(200, `[{"parent_id":1,"parent_role":"parent","inherit_id":2,"inherit_role":"child"}]`), nil
	})

	memberships, err := cloud.GetRoleMemberships(cfg, []string{"public"})
	require.NoError(t, err)
...
func TestGetRoleByNameSuccess(t *testing.T) {
	...
	httpmock.RegisterResponder(http.MethodPost, url, func(req *http.Request) (*http.Response, error) {
		...
		return httpmock.NewStringResponse(200, `[{"name":"role1","config":["search_path=public"]}]`), nil
	})

	role, err := cloud.GetRoleByName(cfg, "role1")
	require.NoError(t, err)
...
func TestGetRoleByNameNotFound(t *testing.T) {
	...
	url := queryURL(cfg)
	httpmock.RegisterResponder(http.MethodPost, url, httpmock.NewStringResponder(200, `[]`))

	role, err := cloud.GetRoleByName(cfg, "missing")
	assert.Error(t, err)
...
func TestCreateRoleSuccess(t *testing.T) {
	...
	httpmock.RegisterResponder(http.MethodPost, url, func(req *http.Request) (*http.Response, error) {
		...
		return httpmock.NewStringResponse(200, `[{"name":"tester","config":["search_path=admin"]}]`), nil
	})

	role, err := cloud.CreateRole(cfg, objects.Role{Name: "tester", Config: map[string]any{"search_path": "admin"}})
	require.NoError(t, err)
...
func TestCreateRoleServerError(t *testing.T) {
	...
	role, err := cloud.CreateRole(cfg, objects.Role{Name: "tester"})
	assert.Error(t, err)
...
func TestDeleteRoleSuccess(t *testing.T) {
	...
	err := cloud.DeleteRole(cfg, objects.Role{Name: "tester"})
...
func TestUpdateRoleNoChanges(t *testing.T) {
	...
	err := cloud.UpdateRole(cfg, objects.Role{Name: ""}, objects.UpdateRoleParam{OldData: objects.Role{Name: "tester"}})
	assert.Error(t, err)
...
func TestUpdateRoleSuccess(t *testing.T) {
	...
	err := cloud.UpdateRole(cfg, objects.Role{Name: ""}, objects.UpdateRoleParam{
